...
router.post('/register', async (req, res) => {
  ...
  if (age < 13) return res.status(400).json({ error: 'Vous devez avoir au moins 13 ans pour vous inscrire' });
  ...
  try {
    User.findByEmail(email, (err, existingEmail) => {
      ...
      User.findByPseudo(pseudo, (err, existingPseudo) => {
        if (err) return res.status(500).json({ error: 'Erreur serveur' });
        if (existingPseudo) return res.status(400).json({ error: 'Ce pseudo est déjà utilisé' });
  ...
  User.findByPseudo(pseudo, (err, user) => {
    ...
    User.verifyPassword(password, user.password, (err, isMatch) => {
      ...
      User.setOnlineStatus(user.id, true, (err) => {
        if (err) console.error('Erreur lors de la mise à jour du statut:', err);
      });
