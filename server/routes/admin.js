...
const db = require('../database'); // <-- ajouté
...
router.put('/users/:id/role', authenticateToken, requireOwner, (req, res) => {
  ...
  User.updateRole(userId, role, badge, (err) => {
    if (err) return res.status(500).json({ error: 'Erreur lors de la modification du rôle' });
...
router.post('/users/:id/verify', authenticateToken, requireOwner, (req, res) => {
  ...
  db.run(
    ...
    [badge, userId],
    function(err) {
      if (err) return res.status(500).json({ error: 'Erreur lors de l\'attribution du badge' });
...
router.delete('/users/:id/verify', authenticateToken, requireOwner, (req, res) => {
  ...
  db.run(
    ...
    [userId],
    function(err) {
      if (err) return res.status(500).json({ error: 'Erreur lors du retrait du badge' });
...
router.post('/rooms', authenticateToken, requireOwner, (req, res) => {
  ...
  if (!name || !topic) return res.status(400).json({ error: 'Nom et sujet du salon requis' });
  ...
  Room.create(
    ...
    (err, room) => {
      if (err) return res.status(500).json({ error: 'Erreur lors de la création du salon' });
...
router.put('/rooms/:id', authenticateToken, requireOwner, (req, res) => {
  ...
  if (isNaN(roomId)) return res.status(400).json({ error: 'ID salon invalide' });

  Room.update(roomId, updates, (err) => {
    if (err) return res.status(500).json({ error: 'Erreur lors de la modification du salon' });
...
router.delete('/rooms/:id', authenticateToken, requireOwner, (req, res) => {
  ...
  Room.delete(roomId, (err) => {
    if (err) return res.status(500).json({ error: 'Erreur lors de la suppression du salon' });
